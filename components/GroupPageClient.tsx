...
import { Copy, Plus, DoorOpen  } from "lucide-react";
			...
			>
				...
				{groupInfo.description.length > 0 && (
					<>
						...
						<p className="">{groupInfo.description}</p>
					</>
				)}
				...
				<ul className="list-disc list-inside ">
					{groupInfo.groupOrganisations.map((organisation: any) => (
						<li key={organisation.id}>@{organisation.username}</li>
					))}
				</ul>

				{groupInfo.organisations.includes(
						currentUser?.organisations[0].id
				...
				{/* <div className="flex items-center mt-4">
					<Button
						...
						onClick={(e) => {
							e.preventDefault();
							copyToClipboard(
								`${window.location.origin}/group/${groupInfo.id}?secret=message`
							);
						}}
					>
