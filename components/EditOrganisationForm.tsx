...
const FormSchema = z.object({
	...
	viewMoreLink: z.string().max(500, {
		message: "Link must be max 500 characters.",
	}),
...
const EditOrganisationForm = ({
	...
	const form = useForm<z.infer<typeof FormSchema>>({
		...
		defaultValues: {
			...
			viewMoreLink: organisationInformation.viewMoreLink,
	...
	async function onSubmit(data: z.infer<typeof FormSchema>) {
		...
		if (
			!isUserNameUnique &&
			data.username !== organisationInformation.username
		) {
			toast({
				...
				<Form {...form}>
					...
					>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>View More Link</FormLabel>
									<FormControl>
										<Input
											placeholder="Link for users to get more information about your organisation"
											{...field}
											className="w-full"
										/>
									</FormControl>
